...
class KukaReachEnv(gym.Env):
    ...
    def reset(self):
        ...
        rebuild = {'observation':np.array(self.current_pos),'achieved_goal':np.array(self.current_pos),'desired_goal':np.array(self.object_pos)}
        return rebuild
    ...
    def step(self, action):
        ...
        '''
        if (temp < 2) & (self.flag == 0):
            self.flag = 1
            p.resetBasePositionAndOrientation(self.object_id, [1, 1, 1], [0, 0, 0, 0])
            self.object_id = p.loadURDF(os.path.join(self.urdf_root_path,
                                                     "random_urdfs/000/000.urdf"),
                                        basePosition=[
                                            random.uniform(self.x_low_obs,
